$(window).on('load', function(){
	...
	if(get_cookie_login!="null")document.getElementById('polygon_area').innerHTML=google.maps.geometry.spherical.computeArea(polygon_list).toFixed(2)+"㎡";
	...
	if(urlCommentAutoSave==1 && (polygon_list.length!=0 || get_comment_text.length!=0) && get_cookie_login!="null"){
...
function getValue() {
	...
	inputflag=true;
	if(urlCommentAutoSave==1)Cookies.set("comment_text", comment_text,{expires:7});
}